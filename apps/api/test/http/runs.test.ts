...
async function projectAgainst(faults: StubFaults, environment: Partial<{ writesAllowed: boolean; authEnforced: boolean; baseUrl: string }> = {}) {
  ...
  await api().post(`${projectBase}/spec-versions`).set(as(owner)).send({ source: { kind: "inline", raw: STUB_SPEC_YAML } });
...
describe("una corrida completa contra un destino correcto", () => {
  ...
  test("el flujo create-read deja la base como estaba", async () => {
    ...
    assert.deepEqual(detail.body.steps.map((step: { purpose: string; method?: string }) => step.purpose), ["act", "verify", "cleanup"]);
    assert.equal(detail.body.steps.every((step: { ok: boolean }) => step.ok), true);
  });
...
describe("permisos y aislamiento", () => {
  ...
  test("un viewer ve las corridas pero no las lanza", async () => {
    ...
    assert.equal((await api().post(`${fixture.projectBase}/runs`).set(as(viewer)).send({ environmentId: fixture.environmentId })).status, 403);
  });
