...
exports.SAFE_FETCH = exports.BlockedTargetError = void 0;
...
exports.BlockedTargetError = BlockedTargetError;
/**
...
function isBlockedAddress(address) {
    ...
    if (family === 4) {
        ...
        for (const { cidr, why } of BLOCKED_V4) {
            ...
            if ((value & mask) >>> 0 === (toLong(network) & mask) >>> 0)
    ...
    if (family === 6) {
        ...
        if (normalized === "::" || normalized === "::1")
            return { blocked: true, why: "loopback IPv6" };
        if (normalized.startsWith("fe80"))
            return { blocked: true, why: "link-local IPv6" };
        if (/^f[cd]/.test(normalized))
            return { blocked: true, why: "unique local IPv6" };
        return { blocked: false };
    }
...
async function resolveTarget(rawUrl, policy) {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    if (url.username || url.password)
        throw new BlockedTargetError(rawUrl, "las credenciales en la URL no se aceptan");
    ...
    if (!policy.allowPrivateTargets) {
        ...
        if (verdict.blocked)
            throw new BlockedTargetError(rawUrl, `${verdict.why} (${address})`);
    }
    return { url, address, family };
}
async function lookupOrFail(hostname, rawUrl) {
    try {
        return await (0, promises_1.lookup)(hostname);
    }
    catch {
...
async function safeFetch(rawUrl, policy, options = {}) {
    ...
    for (let hop = 0; hop <= policy.maxRedirects; hop += 1) {
        if (visited.has(current))
            throw new BlockedTargetError(current, "bucle de redirecciones");
        visited.add(current);
        ...
        try {
            ...
            if (controller.signal.aborted)
                throw new BlockedTargetError(current, `sin respuesta en ${policy.timeoutMs} ms`);
 ...
 */
async function readCapped(response, maxBytes, target) {
...
exports.SAFE_FETCH = Symbol("SAFE_FETCH");
