...
async function signUp(email, organizationName) {
...
async function joinAs(actor, organizationId, role) {
...
const as = (actor) => ({ Authorization: `Bearer ${actor.accessToken}` });
(0, node_test_1.describe)("aislamiento entre organizaciones", () => {
    (0, node_test_1.test)("un ajeno recibe 403 sobre una organización de la que no es miembro", async () => {
        ...
        strict_1.default.equal(response.body.detail, "No perteneces a esta organización");
    });
    (0, node_test_1.test)("un ajeno no puede invitar, cambiar roles ni expulsar", async () => {
        ...
        for (const attempt of attempts)
            strict_1.default.equal((await attempt()).status, 403);
    });
    (0, node_test_1.test)("sin credencial ninguna ruta de organización responde", async () => {
...
(0, node_test_1.describe)("la escalera de roles", () => {
    (0, node_test_1.test)("viewer lee los miembros pero no los gestiona", async () => {
        strict_1.default.equal((await api().get(`/orgs/${owner.organizationId}/members`).set(as(viewer))).status, 200);
        strict_1.default.equal((await api().post(`/orgs/${owner.organizationId}/invitations`).set(as(viewer)).send({ email: "n@example.com", role: "viewer" })).status, 403);
    });
    (0, node_test_1.test)("editor tampoco gestiona miembros ni credenciales", async () => {
        ...
        strict_1.default.equal((await api().post(`/orgs/${owner.organizationId}/invitations`).set(as(editor)).send({ email: "n@example.com", role: "viewer" })).status, 403);
        strict_1.default.equal((await api().get(`/orgs/${owner.organizationId}/tokens`).set(as(editor))).status, 403);
    });
    (0, node_test_1.test)("admin invita, y no por encima de su propio nivel", async () => {
        const allowed = await api().post(`/orgs/${owner.organizationId}/invitations`).set(as(admin)).send({ email: "nuevo@example.com", role: "editor" });
        strict_1.default.equal(allowed.status, 201);
        ...
        strict_1.default.equal(escalation.status, 403);
        strict_1.default.match(escalation.body.type, /role-escalation$/);
    });
    (0, node_test_1.test)("nadie puede cambiar su propio rol", async () => {
        const response = await api().patch(`/orgs/${owner.organizationId}/members/${admin.userId}`).set(as(admin)).send({ role: "owner" });
        strict_1.default.equal(response.status, 403);
        strict_1.default.match(response.body.type, /self-role-change$/);
    });
    (0, node_test_1.test)("un admin no puede degradar a un owner", async () => {
        const response = await api().patch(`/orgs/${owner.organizationId}/members/${owner.userId}`).set(as(admin)).send({ role: "viewer" });
        strict_1.default.equal(response.status, 403);
        strict_1.default.match(response.body.type, /role-escalation$/);
    });
    (0, node_test_1.test)("un rol inexistente es 422 y no un rol silenciosamente ignorado", async () => {
...
(0, node_test_1.describe)("la organización no puede quedarse sin dueño", () => {
    (0, node_test_1.test)("el último owner no puede degradarse ni ser degradado", async () => {
        ...
        strict_1.default.equal(removal.status, 409);
        strict_1.default.match(removal.body.type, /last-owner$/);
    });
    (0, node_test_1.test)("con dos owners, uno puede irse", async () => {
        const second = await signUp("second-owner@example.com");
        await joinAs(second, owner.organizationId, "owner");
        strict_1.default.equal((await api().delete(`/orgs/${owner.organizationId}/members/${second.userId}`).set(as(second))).status, 204);
    });
});
(0, node_test_1.describe)("salir de una organización", () => {
    (0, node_test_1.test)("un viewer puede irse solo, sin permiso para gestionar a nadie", async () => {
        const leaver = await signUp("leaver@example.com");
        await joinAs(leaver, owner.organizationId, "viewer");
        strict_1.default.equal((await api().delete(`/orgs/${owner.organizationId}/members/${leaver.userId}`).set(as(leaver))).status, 204);
        strict_1.default.equal(await context.repositories.memberships.find(owner.organizationId, leaver.userId), null);
    });
    (0, node_test_1.test)("un viewer no puede expulsar a otro", async () => {
        const response = await api().delete(`/orgs/${owner.organizationId}/members/${editor.userId}`).set(as(viewer));
        strict_1.default.equal(response.status, 403);
    });
});
(0, node_test_1.describe)("invitaciones", () => {
    (0, node_test_1.test)("la invitación solo la acepta la dirección a la que se envió", async () => {
        ...
        strict_1.default.equal(wrongPerson.status, 403);
        strict_1.default.match(wrongPerson.body.type, /invitation-wrong-recipient$/);
        ...
        strict_1.default.equal((await context.repositories.memberships.find(owner.organizationId, recipient.userId))?.role, "editor");
    });
    (0, node_test_1.test)("una invitación ya aceptada no vale una segunda vez", async () => {
        ...
        strict_1.default.equal((await api().post("/invitations/accept").set(as(person)).send({ token: invited.body.token })).status, 200);
        strict_1.default.equal((await api().post("/invitations/accept").set(as(person)).send({ token: invited.body.token })).status, 404);
    });
    (0, node_test_1.test)("un token de invitación inventado es 404", async () => {
    ...
    (0, node_test_1.test)("la lista de miembros no expone el hash de ninguna invitación", async () => {
        const response = await api().get(`/orgs/${owner.organizationId}/members`).set(as(viewer));
        strict_1.default.equal(response.status, 200);
        strict_1.default.equal(JSON.stringify(response.body).includes("tokenHash"), false);
    });
});
(0, node_test_1.describe)("tokens de servicio", () => {
    (0, node_test_1.test)("el token se muestra una vez y después solo su prefijo", async () => {
        ...
        strict_1.default.match(created.body.token, /^eqt_/);
        ...
        strict_1.default.equal(JSON.stringify(listed.body).includes(created.body.token), false);
        strict_1.default.equal(listed.body[0].preview, created.body.preview);
    });
    (0, node_test_1.test)("un token de servicio autentica pero no gestiona miembros", async () => {
        const created = await api().post(`/orgs/${owner.organizationId}/tokens`).set(as(owner)).send({ name: "CI" });
        const withToken = { Authorization: `Bearer ${created.body.token}` };
        strict_1.default.equal((await api().get(`/orgs/${owner.organizationId}/members`).set(withToken)).status, 200);
        ...
        strict_1.default.equal(invite.status, 403);
        strict_1.default.match(invite.body.type, /api-token-role$/);
    });
    (0, node_test_1.test)("un token de servicio no puede actuar sobre otra organización", async () => {
        const created = await api().post(`/orgs/${owner.organizationId}/tokens`).set(as(owner)).send({ name: "CI" });
        const response = await api().get(`/orgs/${outsider.organizationId}/members`).set({ Authorization: `Bearer ${created.body.token}` });
        strict_1.default.equal(response.status, 403);
    });
    (0, node_test_1.test)("un token revocado deja de autenticar", async () => {
        const created = await api().post(`/orgs/${owner.organizationId}/tokens`).set(as(owner)).send({ name: "temporal" });
        strict_1.default.equal((await api().delete(`/orgs/${owner.organizationId}/tokens/${created.body.id}`).set(as(owner))).status, 204);
        const response = await api().get(`/orgs/${owner.organizationId}/members`).set({ Authorization: `Bearer ${created.body.token}` });
        strict_1.default.equal(response.status, 401);
    });
    (0, node_test_1.test)("revocar un token de otra organización es 404 y no 403", async () => {
