...
export class ExportProjectHandler implements IQueryHandler<ExportProjectQuery, ProjectBundle> {
  ...
  async execute(query: ExportProjectQuery): Promise<ProjectBundle> {
    ...
    if (parts.has("flows")) bundle.flows = await this.flows(project.id, query.workflowIds);
  ...
  private async flows(projectId: string, workflowIds: string[]): Promise<NonNullable<ProjectBundle["flows"]>> {
