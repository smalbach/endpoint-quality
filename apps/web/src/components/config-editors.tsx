...
const field = "h-8 rounded-lg border border-slate-200 px-2 text-xs outline-none focus:border-slate-900 disabled:bg-slate-50";
      ...
      {rows.map((row, index) => (
        <div key={String(row.id ?? index)} className="flex flex-wrap items-end gap-2 border-b border-slate-100 py-2">
          ...
          <span className="ml-auto flex items-center gap-1">
            <Button variant="ghost" className="h-7 w-7 p-0 text-xs" disabled={disabled || index === 0} title="Subir" onClick={() => onChange(move(rows, index, -1))}>
              ↑
            </Button>
            <Button
              variant="ghost"
            ...
            <Button variant="ghost" className="h-7 px-2 text-xs text-rose-600" disabled={disabled} onClick={() => onChange(removeAt(rows, index))}>
              Quitar
            </Button>
          </span>
        </div>
      ))}
      <Button variant="ghost" className="mt-2 h-7 px-2 text-xs" disabled={disabled} onClick={() => onChange([...rows, onAdd()])}>
        {addLabel}
      </Button>
    </div>
  );
...
function Methods({ value, onChange, disabled }: { value: HttpMethod[]; onChange: (next: HttpMethod[]) => void; disabled: boolean }) {
  return (
    <div>
      <p className={label}>Métodos {value.length === 0 && <span className="normal-case text-slate-400">· cualquiera</span>}</p>
      ...
      {entries.map(([key, entry], index) => (
        <div key={index} className="flex items-end gap-2 py-1">
          <div>
            ...
            <input className={`${field} w-52 font-mono`} value={key} disabled={disabled} onChange={(event) => write(entries.map((pair, position) => (position === index ? [event.target.value, entry] : pair)) as [string, string][])} />
          </div>
          <div className="flex-1">
            <p className={label}>{valueLabel}</p>
            <input className={`${field} w-full font-mono`} value={entry} disabled={disabled} onChange={(event) => write(entries.map((pair, position) => (position === index ? [key, event.target.value] : pair)) as [string, string][])} />
          </div>
          <Button variant="ghost" className="h-8 px-2 text-xs text-rose-600" disabled={disabled} onClick={() => write(entries.filter((_pair, position) => position !== index) as [string, string][])}>
            Quitar
          </Button>
        </div>
      ))}
      <Button variant="ghost" className="mt-1 h-7 px-2 text-xs" disabled={disabled} onClick={() => onChange({ ...value, "": "" })}>
        Añadir
      </Button>
    </div>
  );
...
function StringLines({ value, onChange, disabled, hint }: { value: string[]; onChange: (next: string[]) => void; disabled: boolean; hint: string }) {
  return (
    <div>
...
function BudgetsEditor({ value, onChange, disabled }: EditorProps) {
  ...
  return (
    <RuleRows
      ...
      onAdd={() => ({ id: slugId("presupuesto", rows.map((row) => String(row.id))), thresholdMs: 500, label: "Presupuesto", source: "manual" })}
      onChange={(next) => onChange({ ...value, budgets: next.map((row) => compact(row)) })}
      render={(row, update) => (
        <>
          <div>
            <p className={label}>Etiqueta</p>
            <input className={`${field} w-40`} value={String(row.label ?? "")} disabled={disabled} onChange={(event) => update({ label: event.target.value })} />
          </div>
          <div>
          ...
          <Methods value={(row.methods as HttpMethod[]) ?? []} disabled={disabled} onChange={(methods) => update({ methods })} />
          <div>
            <p className={label}>La ruta acaba en</p>
            <input className={`${field} w-32 font-mono`} value={String(row.pathSuffix ?? "")} disabled={disabled} onChange={(event) => update({ pathSuffix: event.target.value })} />
          </div>
          <div>
            <p className={label}>Origen</p>
            <input className={`${field} w-28`} value={String(row.source ?? "")} disabled={disabled} onChange={(event) => update({ source: event.target.value })} />
          </div>
        </>
      )}
    />
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-3">
        <div>
          ...
          <input className={`${field} w-40 font-mono`} value={String(envelope.fallbackShape ?? "")} disabled={disabled} onChange={(event) => write({ fallbackShape: event.target.value })} />
        </div>
        <div>
          <p className={label}>Forma de un error</p>
          <input className={`${field} w-40 font-mono`} value={String(envelope.errorShape ?? "")} disabled={disabled} onChange={(event) => write({ errorShape: event.target.value })} />
        </div>
      </div>
      <RuleRows
        rows={rows}
        ...
        onAdd={() => ({ id: slugId("envelope", rows.map((row) => String(row.id))), match: {}, shape: String(envelope.fallbackShape ?? "data") })}
        ...
        render={(row, update) => {
          ...
          return (
            <>
              <Methods value={(match.methods as HttpMethod[]) ?? []} disabled={disabled} onChange={(methods) => patchMatch({ methods })} />
              <div>
                <p className={label}>La operación empieza por</p>
                <input className={`${field} w-32 font-mono`} value={String(match.operationIdPrefix ?? "")} disabled={disabled} onChange={(event) => patchMatch({ operationIdPrefix: event.target.value })} />
              </div>
              <div>
                <p className={label}>La ruta acaba en</p>
                <input className={`${field} w-28 font-mono`} value={String(match.pathSuffix ?? "")} disabled={disabled} onChange={(event) => patchMatch({ pathSuffix: event.target.value })} />
              </div>
              <div>
                <p className={label}>Forma</p>
                <input className={`${field} w-32 font-mono`} value={String(row.shape ?? "")} disabled={disabled} onChange={(event) => update({ shape: event.target.value })} />
              </div>
            </>
          );
        }}
  ...
  if (current === null) {
    return (
      <div className="text-xs text-slate-500">
        ...
        <Button variant="ghost" className="mt-2 h-7 px-2 text-xs" disabled={disabled} onClick={() => onChange({ ...value, implemented: operationIds })}>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <input className={`${field} w-48`} placeholder="Filtrar" value={filter} disabled={disabled} onChange={(event) => setFilter(event.target.value)} />
        <span className="text-[11px] text-slate-500">
          {current.length} de {operationIds.length}
        </span>
        <Button variant="ghost" className="h-7 px-2 text-xs" disabled={disabled} onClick={() => onChange({ ...value, implemented: operationIds })}>
          Todas
        </Button>
        <Button variant="ghost" className="h-7 px-2 text-xs" disabled={disabled} onClick={() => onChange({ ...value, implemented: [] })}>
          Ninguna
        </Button>
        <Button variant="ghost" className="ml-auto h-7 px-2 text-xs" disabled={disabled} onClick={() => onChange({ ...value, implemented: null })}>
          Volver a «sin decidir»
        </Button>
      </div>
      <div className="mt-2 grid max-h-64 grid-cols-1 gap-x-4 overflow-y-auto sm:grid-cols-2">
        {operationIds.length === 0 && <p className="text-xs text-slate-400">Importa un contrato y aquí saldrán sus operaciones.</p>}
        {shown.map((id) => (
          <label key={id} className="flex items-center gap-2 py-0.5 font-mono text-[11px] text-slate-700">
            <input
              ...
              disabled={disabled}
              onChange={(event) => onChange({ ...value, implemented: event.target.checked ? [...current, id] : current.filter((entry) => entry !== id) })}
            />
            {id}
          </label>
        ))}
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-3">
        <div>
          ...
          <input className={`${field} w-32 font-mono`} value={String(value.fallbackPathValue ?? "")} disabled={disabled} onChange={(event) => onChange({ ...value, fallbackPathValue: event.target.value })} />
        </div>
        <div>
          <p className={label}>Identificador que no existe</p>
          <input className={`${field} w-32 font-mono`} value={String(value.missingIdValue ?? "")} disabled={disabled} onChange={(event) => onChange({ ...value, missingIdValue: event.target.value })} />
        </div>
      </div>
      <div>
        <p className="text-[11px] text-slate-500">Valores concretos por parámetro de ruta. Es lo que separa un 404 de verdad de uno que solo dice que el identificador era inventado.</p>
        <StringMap
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <input className={`${field} w-56 font-mono`} value={String(scopes.default ?? "")} disabled={disabled} onChange={(event) => onChange({ ...value, scopes: { ...scopes, default: event.target.value } })} />
      </div>
      <RuleRows
        rows={rows}
        disabled={disabled}
        ...
        addLabel="Añadir regla"
        onAdd={() => ({ id: slugId("auth", rows.map((row) => String(row.id))), credential: "none", expectedStatus: 401, when: {} })}
          ...
          return (
            <>
              <div>
                ...
                <select className={`${field} w-28`} value={String(row.credential ?? "none")} disabled={disabled} onChange={(event) => update({ credential: event.target.value })}>
              ...
              <div>
                ...
                <input className={`${field} w-20`} type="number" value={Number(row.expectedStatus ?? 401)} disabled={disabled} onChange={(event) => update({ expectedStatus: Number(event.target.value) })} />
              </div>
              <div>
                ...
                <input
                  ...
                  disabled={disabled}
                  onChange={(event) => update({ when: compact({ ...when, declaredStatus: event.target.value ? Number(event.target.value) : undefined }) })}
                />
              </div>
              <Methods value={(when.methods as HttpMethod[]) ?? []} disabled={disabled} onChange={(methods) => update({ when: compact({ ...when, methods }) })} />
            </>
          );
        }}
      />
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <select className={`${field} w-24`} value={String(value.locale ?? "es")} disabled={disabled} onChange={(event) => onChange({ ...value, locale: event.target.value })}>
